...
(function () {
  ...
  const scriptEl = document.currentScript || document.querySelector('script[data-resource]');
  ...
  function showLoading() { document.documentElement.style.visibility = 'hidden'; }
  function hideLoading() { document.documentElement.style.visibility = 'visible'; }
  ...
  function showRequestForm(reason) {
      ...
      <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .guard-box{max-width:480px;width:100%;border:1px solid rgba(0,212,255,0.2);
          background:linear-gradient(135deg,rgba(4,13,30,0.98),rgba(6,15,28,0.98));
        ...
        input,textarea{width:100%;background:rgba(255,255,255,0.04);
          border:1px solid rgba(0,212,255,0.15);color:#E2E8F0;padding:10px 14px;
          font-family:'Inter',sans-serif;font-size:.85rem;border-radius:2px;outline:none}
    ...
    // Listener fuera del innerHTML — los scripts dentro de innerHTML no ejecutan
  ...
  async function validate() {
    ...
    try {
      ...
      console.warn('[shellti-guard] Error de conexión:', err.message);
      hideLoading();
    }
  }
